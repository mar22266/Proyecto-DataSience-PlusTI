...
simulados y se compararon contra modelos centralizados. Además, las correcciones recibidas sobre el primer 30\% de Banco~3 se usaron para seleccionar
la metodología final aplicada al 100\% del banco objetivo.
...
Primero se realizó una selección interna usando Banco~1 y Banco~2 mediante puntuaciones
compuestas que combinaban F1 promedio entre bancos, F1 mínimo entre bancos, ROC AUC,
recall y penalización por false positive ratio. Esta selección permitió identificar
el mejor modelo bajo validación interna.

Después, las correcciones recibidas sobre el primer 30\% de Banco~3 se utilizaron
como criterio específico del banco objetivo. La metodología con mejor desempeño corregido
en ese subconjunto fue la usada para generar la inferencia final sobre el 100\% de Banco~3.
...
simulados. Para la selección interna  se compararon seis estrategias candidatas sobre los
...
En validación interna con Banco~1 y Banco~2, la mejor estrategia fue
\texttt{central\_hist\_gradient\_boosting\_model} con umbral 0.85. Los ensambles
federados simulados permitieron comparar transferencia entre bancos, pero no superaron
al mejor modelo centralizado bajo esta validación interna. Sin embargo, esta selección
se revisó posteriormente con las correcciones recibidas sobre el primer 30\% de Banco~3.

\begin{center}
\scriptsize
\begin{tabular}{p{0.34\textwidth}rrrrr}
\toprule
\textbf{Modelo} & \textbf{Accuracy} & \textbf{Prec.} & \textbf{Recall} & \textbf{F1} & \textbf{Ratio FP} \\
\midrule
Baseline centralizado & 97.20\% & 98.44\% & 34.62\% & 51.22\% & 1.56\% \\
Federado pesos iguales & 97.84\% & 91.84\% & 53.92\% & 67.95\% & 8.16\% \\
Federado ponderado (Sin correcciones de PlusTi) & -- & -- & -- & -- & -- \\
\bottomrule
\end{tabular}
\end{center}

El modelo federado con pesos iguales obtuvo el mejor F1, recall y accuracy sobre las
correcciones recibidas del primer 30\% de Banco~3. El modelo federado ponderado no pudo
compararse en esta etapa porque no se recibió corrección para esa entrega preliminar.
...
el 100\% de Banco~3 se usó \texttt{federated\_equal\_weight\_model} con umbral 0.60,
al ser la mejor metodología según las correcciones recibidas del primer 30\%.
El modelo predijo 1{,}778 fraudes sobre 100{,}000 transacciones, equivalente a una
tasa esperada de 1.78\%.
...
La inferencia fue generada con \texttt{federated\_equal\_weight\_model} y umbral 0.60.
Esta elección se basó en las correcciones recibidas sobre el primer 30\% de Banco~3,
no únicamente en la validación interna con Banco~1 y Banco~2.
...
únicamente de la información de un banco. En la validación interna con Banco~1 y Banco~2,
el modelo centralizado HistGradientBoosting obtuvo el mejor desempeño. Sin embargo, al
incorporar las correcciones recibidas sobre el primer 30\% de Banco~3, el ensamble federado
con pesos iguales mostró mejor desempeño sobre el banco objetivo. Por esa razón,
\texttt{federated\_equal\_weight\_model} fue la metodología usada para generar la
inferencia final de Banco~3.
...
en un único entorno local, sin comunicación real de parámetros entre nodos; y (3)~la ausencia de etiqueta 
confiable para el 100\% de Banco~3, que impide evaluar completamente la calidad final de las inferencias generadas.
...
\begin{itemize}[leftmargin=*, itemsep=0pt]
  ...
  \item Resumen esperado Banco~3: \nolinkurl{outputs_fase_B/07_reports/expected_result_summary.json}
  \item Inferencia final Banco~3: \nolinkurl{outputs_fase_B/05_predictions/bank_3_full_final_inference.xlsx}
  \item Validación interna en Banco~1 y Banco~2 del modelo usado para Banco~3: \nolinkurl{outputs_fase_B/07_reports/bank_3_final_strategy_internal_metrics.csv}
  \item Correcciones preliminares Banco~3: \nolinkurl{data/corregido}
\end{itemize}
